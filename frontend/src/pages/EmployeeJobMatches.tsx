...
import { useProjects, useProjectsFull } from "@/api/hooks";
...
export default function EmployeeJobMatches() {
  ...
  const recommended = useMemo(() => {
    if (!projects.length || !userSkills.length) return [];

    return [...projects]
      .map((p) => ({
        project: p,
        match: computeMatch(userSkills, p).percentage,
      }))
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {userSkills.length
                ? `Projects aligned with your skills: ${userSkills.join(", ")}`
                : "No skills found for current user"}
            </p>
          </div>
        </div>
      </header>
          ...
          <section>
            ...
            <div className="relative max-w-md mb-6">
              ...
              <Input
                ...
                placeholder="Search by project name, domain, skill, or role..."
            ...
            {search.trim() ? (
              ...
              <p className="text-sm text-muted-foreground">
                Start typing to search across all available projects.
              </p>
            )}
          </section>
        </div>
